...
def capitalize_columns(df_data) -> pd.core.frame.DataFrame:
    ...
    for columnName in new_df.columns:
        if 'e' not in columnName:
            columns[columnName] = columnName.upper()

    df_renamed = new_df.rename(columns=columns)
    return df_renamed
...
def math_passed_count(df_data) -> int:
    new_df = df_data.copy()
    df = new_df[new_df['math score'] >= 50]
    return len(df)
...
def did_pre_course(df_data) -> pd.core.frame.DataFrame:
    new_df = df_data.copy()
    df = new_df[new_df['test preparation course'] == 'completed']
    return df
...
def female_top_score(df_data):
    ...
    female_df = female_df.sort_values(by=['math score', 'reading score', 'writing score'], ascending=[False, False, False])
    tpl = (int(female_df[:1]['math score']),int(female_df[:1]['reading score']),int(female_df[:1]['writing score']))
    return tpl
...
def add_grade(df_data):
    ...
    for i in range(len(new_df)):
        percent = (int(new_df['math score'][i]) + int(new_df['reading score'][i]) + int(new_df['writing score'][i])) / 3
        if percent < 60:
            new_df['grade'][i] = 'F'
        elif percent < 70:
            new_df['grade'][i] = 'D'
