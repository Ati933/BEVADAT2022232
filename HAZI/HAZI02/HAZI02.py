...
def column_swap(inpt: np.array) -> np.array:
    return np.fliplr(inpt)
...
def compare_two_array(inpt1: np.array, inpt2: np.array) -> np.array:
    return np.where(inpt1 == inpt2)
...
def get_array_shape(inpt: np.array) -> str:
    shape = np.shape(inpt)

    if len(shape) == 1:
        return "sor: " + str(1) + ", oszlop: " + str(shape[0]) + ", melyseg: " + str(1)
    elif len(shape) == 2:
        return "sor: " + str(shape[0]) + ", oszlop: " + str(shape[1]) + ", melyseg: " + str(1)
    
    return "sor: " + str(shape[0]) + ", oszlop: " + str(shape[2]) + ", melyseg: " + str(shape[1])
...
def decode_Y(inpt: np.array) -> np.array:
    return np.where(inpt == 1)[1]
...
def eval_classification(inpt1: list, inpt2: np.array):
    return inpt1[np.argmax(inpt2)]
...
def array_multi_2d(inpt: np.array) -> np.array:
    return np.prod(inpt, axis=1)
...
def add_border(inpt: np.array) -> np.array:
    return np.pad(inpt, 1, mode='constant', constant_values=0)
...
def list_days(start: str, end: str) -> np.array:
    interval = np.datetime64(end + '-01') - np.datetime64(start + '-01')
    return np.array(np.arange(start + '-01', interval.astype(int), 1, dtype='M8[D]'))
